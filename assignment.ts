...
export interface Member {
  ...
  export const createIntroduction = (member: Member): string => {
    ...
    let something:string = `안녕하세요, 저는 ${member.name}이고 기수는 ${member.batch}기입니다. ${member.isActive ? "활동위원입니다!" : "명예위원입니다!"}`
    return something;
  };
  ...
  export const oddNumbers = numbers.filter(m => m % 2 === 1)
  
  // numbers 배열을 이용해 모든 숫자를 2배한 새 배열을 만드세요.
  export const doubledNumbers =  numbers.map(m => m* 2)
  ...
  // numbers 배열의 합을 구하세요.
  export const sumOfNumbers = numbers.reduce((c, r ) => c + r)
  ...
  export const printUserData = async (): Promise<{ id: number; name: string }[]> => {
    ...
    var idkwtfisthis = await fetchUserData();
    console.log(idkwtfisthis);
    return idkwtfisthis;
